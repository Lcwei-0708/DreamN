        ...
        try:
            ...
            if response.status_code in (401, 403):
                try:
                    ...
                    if api_fails == 1:
                        await redis.expire(api_fail_key, FAIL_WINDOW)
                    
                    if api_fails >= FAIL_LIMIT:
                        logger.warning(f"IP {ip} is now blocked for API {path} for {BLOCK_TIME} seconds")
                        await redis.set(api_block_key, 1, ex=BLOCK_TIME)
                        await redis.delete(api_fail_key)
